...
func (s *handshakeTestSuite) TestHandshakeInit() {
	// Test successful init
	s.T().Run("successful init", func(t *testing.T) {
...
func (s *handshakeTestSuite) TestHandshakeCheck() {
	// Test successful check with token
	s.T().Run("successful check with token", func(t *testing.T) {
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			// Verify request
			assert.Equal(t, "POST", r.Method)
	...
	// Test successful check without token
	s.T().Run("successful check without token", func(t *testing.T) {
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			w.WriteHeader(http.StatusOK)
			w.Write([]byte(`{"encodedId": "test-handshake-id"}`))
		}))
